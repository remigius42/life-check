...
Expose a privacy-bounded binary HA status sensor from the ESP32 firmware via the ESPHome native API, so Home Assistant can automate on threshold-crossing events without receiving raw activity counts or nighttime data.
