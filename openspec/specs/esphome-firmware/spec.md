...
When `web_server_version` is `"3"`, the web UI SHALL define six `sorting_groups`: **Status** (Beam interruption sensor, Today's Break Count, Daily History), **Controls** (Test Mode, Reset Today's Count), **Report** (Break Threshold, Report Hour, Report Minute), **Messages** (Message: Zero, Message: Low, Message: OK), **Webhook** (Webhook URL, Webhook Retries), **System** (Reset to Defaults, Firmware Version). Every entity SHALL declare a `sorting_group_id` and sequential `sorting_weight` (1, 2, 3…) within its group. These blocks SHALL be commented out when running version 2 (they hard-error under v2).
