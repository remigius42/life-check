...
- [x] 1.2 Add `api:` component with `encryption.key: !secret api_encryption_key`
  to enable the ESPHome native API transport required for HA discovery; add
  `api_encryption_key` to `secrets.yaml.example` with generation instructions
- [x] 1.3 Add `ha_status_sensor` binary_sensor (ON when `today_count` ≥
  `break_threshold`)
- [x] 1.4 Add jitter script (`mode: single`, delay `900 + rand() %
...
- [x] 1.8 Add substitutions `ha_privacy_window_end_hour: "8"` and
  `ha_privacy_window_end_minute: "0"` with comment that they must not become UI
  entities
- [x] 1.9 Add inline privacy window lambda (midnight cap logic using
...
- [x] 1.10 Add `on_time` at `ha_privacy_window_end_hour:ha_privacy_window_end_minute`
  seconds:0 that publishes `today_count >= break_threshold` without jitter
- [x] 1.11 Extend existing per-minute `on_time` at `report_time`: add
...
- [x] 4.0 Fix CI pipeline: add "Create dummy ESPHome secrets" step to
  `.github/workflows/ci.yml` that copies `secrets.yaml.example` and substitutes a
  valid dummy base64 value for `api_encryption_key` so `esphome-config-validate`
  passes without requiring a real secrets file

## 5. Release

- [x] 5.1 Add entry to `CHANGELOG.md` under a new minor version (2.x.0 — check
  current version at implementation time and bump the minor component)
- [x] 5.2 Update `esphome.project.version` in `esphome/life-check.yaml` to match
