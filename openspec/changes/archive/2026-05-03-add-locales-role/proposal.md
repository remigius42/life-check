...
The life_man_switch Ansible project has no roles or playbooks yet. Adding the `locales` role bootstraps the project with
its first deployable capability: consistent locale and timezone configuration on the target Raspberry Pi.
...
- `locales`: Installs the `locales` package, enables specified locales in `/etc/locale.gen`, and sets the system
