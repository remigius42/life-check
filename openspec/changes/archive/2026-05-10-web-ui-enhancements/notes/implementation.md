...
- `_read_today_count()` currently only returns an `int`. It should be refactored to return the full history dictionary
  or at least the part needed for rendering.
- `index()` HTML template:
...
- The `onmessage` JS handler only updates specific IDs (`beam`, `count`, `mode`). It does not need to change to support
  history (history is static per day).
