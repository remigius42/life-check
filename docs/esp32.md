...
# Edit esphome/secrets.yaml with your WiFi credentials, webhook URL, and API key
# Generate the API encryption key with: openssl rand -base64 32
...
API (Noise protocol, AES-128-GCM). Set `api_encryption_key` in your
`secrets.yaml` before flashing (generate with `openssl rand -base64 32`), then
add the device in the Home Assistant UI (Settings → Devices & Services → Add
Integration → ESPHome) — no further manual HA configuration is required.
